...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("GiGiDesk".to_owned());
...
// GIGI_WS_DOMAIN / check_ws() (websocket.rs) are ready for a wss://…/ws/id
// fallback on port 443 (via nginx, see docs/infra) for restrictive networks
// that block the raw TCP/UDP ports (21115-21119) — the same 443-fallback
// pattern TeamViewer/AnyDesk use. NOT added to RENDEZVOUS_SERVERS yet:
// registering the same peer ID concurrently via two transports (direct TCP
// to the IP + wss-via-nginx to the domain) causes hbbs to reject one of
// them ("unknown RegisterPkResponse" / "Rendezvous connection is reset by
// the peer"), confirmed live against the real server. Wiring this up needs
...
// Gated the same way as RENDEZVOUS_SERVERS/RS_PUB_KEY below: staging and
// production are two independent relays with two independent WSS domains,
// see docs/infra/plan/domain-change.md.
#[cfg(feature = "env_production")]
pub const GIGI_WS_DOMAIN: &str = "rustdesk.gigisquad.com";
#[cfg(not(feature = "env_production"))]
pub const GIGI_WS_DOMAIN: &str = "staging.rustdesk.gigisquad.com";
...
// docs/infra/production/rustdesk.md) — production is NOT staging's box under
